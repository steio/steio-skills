---
name: eval-improve
description: Analyze eval results, diagnose failures, apply targeted fixes, and re-run to verify improvements. Use when debugging evaluation scores, fixing failing or regressed criteria, improving tile or context file content after an eval run, or iterating on agent performance test results.
---
...
**Eval type matters:** This skill handles two kinds of evals:
- **Tile evals** — the eval tests how well a tile teaches agents. Fixes go in the tile's `SKILL.md`, `rules/*.md`, and `docs/*.md`.
- **Codebase evals** — the eval tests how well context files (CLAUDE.md, AGENTS.md, etc.) in the target repo improve agent performance. Fixes go in those context files in the target repo.

Ask the user upfront if you're unsure which type you're working with.
...
> "No eval scenarios found. To use eval-improve, you first need scenarios to evaluate against. You can set these up with the `eval-setup` skill, which will:
> 1. Browse your repo's recent commits
> 2. Generate eval scenarios from real diffs
> 3. Download them to disk
> 4. Run baseline + with-context evals
>
...
```bash
tessl eval compare ./evals/ --breakdown --workspace <workspace> --json
```

Note: `--workspace` is required for `eval compare` but NOT for `eval view`.

The `eval view` gives you the detailed per-criterion scores. The `eval compare --breakdown` gives you the aggregate baseline vs. with-context comparison across all scenarios, color-coded by performance tier (green >= 80%, yellow >= 50%, red < 50%).
...
**For codebase evals**, read:
- `CLAUDE.md` / `AGENTS.md` / `GEMINI.md` in the target repo
- `.github/copilot-instructions.md`
- Any `.mdc` rule files
- The scenario's `scenario.json` to confirm which repo and commit the fixture uses

### 2.3 Find the gap

For each failing criterion, determine:
- **What the rubric wants**: The specific behavior or content the judge is looking for
- **What the context says**: What guidance the context files currently provide (or don't)
- **The gap**: What's missing, vague, or contradictory

### 2.4 Check for contradictions

Scan across ALL relevant files for statements that contradict each other. Common patterns:
...
### 3.3 Lint after each fix (tile evals only)
...
**For tile evals**, commit the tile files:
```bash
git add skills/ rules/ docs/ evals/
git commit -m "Improve tile: <brief description of fixes>"
```

**For codebase evals**, commit the context files in the target repo:
```bash
git add CLAUDE.md AGENTS.md .github/ evals/
git commit -m "Improve context: <brief description of fixes>"
```
...
Note: `--workspace` is required. The eval run will use the latest committed version of your context files. If the eval doesn't pick up your changes, make sure you've committed them first.
...
Wait until status shows completed. With N scenarios, expect ~N × 10–15 minutes. Then get both detailed and aggregate results:
...
The `compare --breakdown` output gives you the full picture across all scenarios with color-coded scores (green >= 80%, yellow >= 50%, red < 50%).
